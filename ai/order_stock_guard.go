...
func lookupCatalogStockLines(ctx context.Context, q tenantQuerier, catalogItemID string) (tracked bool, lines []catalogStockLine) {
	...
	rows, err := q.QueryContext(ctx, `
		...
		ORDER BY w.is_default DESC, w.display_order, w.name`, catalogItemID)
	if err != nil {
		return false, nil
	}
	defer rows.Close()
...
func ensureDraftOrderStock(ctx context.Context, q tenantQuerier, st orderState) (reject bool, reply string, warehouseID string) {
	if st.Qty < 1 || strings.TrimSpace(st.CatalogItemID) == "" {
		return false, "", ""
	}
	tracked, lines := lookupCatalogStockLines(ctx, q, st.CatalogItemID)
	if !tracked {
		return false, "", ""
	}
	wh, ok := resolveOrderWarehouse(lines, st.Qty, st.WarehouseID)
