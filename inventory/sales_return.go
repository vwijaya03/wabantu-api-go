...
func CreateSalesReturn(ctx context.Context, p *CreateSalesReturnParams) (*SalesReturn, error) {
	...
	// Validate returnable quantities before writing anything.
	...
	if err := tx.QueryRowContext(ctx, `
		...
		RETURNING id`,
		returnNo, nullUUID(p.OrderID), nullUUID(contactID), nullStr(p.Note), nullUUID(u.AccountID)).Scan(&returnID); err != nil {
		return nil, appErrs.Internal(err.Error())
	}
	...
	for _, l := range p.Lines {
		...
		unitCost := weightedItemCost(costByItem, l.CatalogItemID) // original HPP from the sale
	...
	// Accrual mode: returned goods reduce COGS -> post income against HPP/COGS.
