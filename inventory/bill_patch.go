...
func UpdateBill(ctx context.Context, id string, p *UpdateBillParams) (*Bill, error) {
	...
	existing, err := getBill(ctx, sch, pool, id)
	if err != nil {
		return nil, err
	}
	...
	if err := revertPOReceiptsForBill(ctx, sch, tx, id); err != nil {
		return nil, err
	}
	if _, err := purgeMovementsByRef(ctx, sch, tx, "bill", id); err != nil {
		return nil, err
	}
	if _, err := qexec(ctx, sch, tx, `DELETE FROM pur_bill_line WHERE bill_id = $1`, id); err != nil {
		return nil, appErrs.Internal(err.Error())
	}
	...
	if err := postBillLinesTx(ctx, sch, tx, id, existing.BillNo, poID, u.AccountID, p.Lines); err != nil {
		return nil, err
	}
...
func revertPOReceiptsForBill(ctx context.Context, sch appdb.SchemaSQL, tx *sql.Tx, billID string) error {
...
func refreshPOStatusTx(ctx context.Context, sch appdb.SchemaSQL, tx *sql.Tx, poID string) error {
...
func postBillLinesTx(ctx context.Context, sch appdb.SchemaSQL, tx *sql.Tx, billID, billNo, poID, accountID string, lines []BillLineInput) error {
	for _, l := range lines {
		...
		if err := ensureSku(ctx, sch, tx, l.CatalogItemID); err != nil {
			return appErrs.Internal(err.Error())
		}
	...
	if strings.TrimSpace(poID) != "" {
		return refreshPOStatusTx(ctx, sch, tx, poID)
	}
	return nil
}
